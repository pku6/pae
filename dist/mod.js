...
async function main() {
    ...
    async function getCourseInfoArray(cookie) {
        for (let i = 0; i < init_1.config.errLimit; i++) {
            try {
                ...
                (0, fs_1.writeFileSync)((0, path_1.join)(__dirname, `../info/invalid-html/${clit_1.ECLIT.getDate()}-${clit_1.ECLIT.getTime().replace(/:/g, '-')}.html`), body);
    ...
    async function getVCodeImg(cookie) {
        ...
        (0, fs_1.writeFileSync)((0, path_1.join)(__dirname, `../info/vcode-imgs/${clit_1.ECLIT.getDate()}-${clit_1.ECLIT.getTime().replace(/:/g, '-')}.gif`), buffer);
    ...
    async function recognizeVCodeImg(base64Img) {
        const { body } = await post('https://api.ttshitu.com/base64', {
            ...
            typeid: init_1.config.recognizeType,
            image: base64Img
    ...
    async function electCourse(href, cookie) {
        try {
            ...
            (0, fs_1.writeFileSync)((0, path_1.join)(__dirname, `../info/election-results/${clit_1.ECLIT.getDate()}-${clit_1.ECLIT.getTime().replace(/:/g, '-')}.html`), body);
            if (body.includes('会话超时') || body.includes('超时操作') || body.includes('重新登录')) {
                clit.out('Expired');
                return 401;
            }
